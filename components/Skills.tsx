...
const Skills: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const generatePoint = (hour: number) => ({
      time: `${hour % 24}:00`,
      value: 40 + (Math.random() * 20 + Math.sin(hour / 2) * 10) * intensity,
      temp: 60 + (Math.random() * 10 + Math.cos(hour / 3) * 5) * intensity
    });
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="absolute top-0 right-0 p-4">
            <div className={`flex items-center gap-2 px-3 py-1 rounded-full transition-colors border text-[10px] font-bold uppercase tracking-tighter ${isPaused ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
          ...
          <div className="flex flex-wrap gap-6 mb-6">
            <div className="flex flex-col gap-2">
              <span className="text-[10px] uppercase font-bold text-slate-500 tracking-widest">Controls</span>
              <button
                onClick={() => setIsPaused(!isPaused)}
                aria-pressed={isPaused}
                aria-label={isPaused ? "Resume simulation" : "Pause simulation"}
                className={`flex items-center gap-2 px-4 py-1 rounded text-[10px] font-bold transition-all border focus-visible:ring-2 focus-visible:ring-primary-500 outline-none ${isPaused
