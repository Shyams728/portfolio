...
const Skills: React.FC = () => {
    ...
    useEffect(() => {
        const statuses = ['SCANNING...', 'CALCULATING...', 'ANALYZING...', 'SYNCING...', 'Ready'];
        const interval = setInterval(() => {
            if (Math.random() > 0.7) {
                const randomStatus = statuses[Math.floor(Math.random() * (statuses.length - 1))];
                setDiagnosticStatus(randomStatus);
                setTimeout(() => setDiagnosticStatus('Ready'), 2000);
            }
        }, 5000);
        ...
        <section id="expertise" className="py-24 bg-industrial-steel/40 relative overflow-hidden border-y border-slate-200 dark:border-slate-900/50 transition-colors">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
                    {/* TOP LEFT: Tech Arsenal */}
                    <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        className="relative glass p-8 border border-slate-200 dark:border-slate-800 rounded-sm"
                    >
                        <div className="flex items-center gap-4 mb-12">
                            <div className="h-[3px] w-12 bg-industrial-orange"></div>
                            <h3 className="text-2xl font-heading font-black text-industrial-concrete dark:text-white uppercase tracking-tight transition-colors">
                    ...
                    {/* TOP RIGHT: Domain Mastery */}
                    <motion.div
                        initial={{ opacity: 0, x: 20 }}
                        whileInView={{ opacity: 1, x: 0 }}
                        viewport={{ once: true }}
                        className="glass p-8 border border-slate-200 dark:border-slate-800 rounded-sm"
                    >
                        <div className="flex items-center gap-4 mb-10">
                            <div className="h-[3px] w-12 bg-industrial-blue"></div>
                            <h3 className="text-2xl font-heading font-black text-industrial-concrete dark:text-white uppercase tracking-tight transition-colors">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {domainData.map((domain, i) => (
                                <motion.div
                                    ...
                                    className="bg-slate-100/30 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 p-4 group hover:bg-slate-50 dark:hover:bg-slate-900 transition-all hover:border-industrial-orange"
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="p-1.5 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-sm text-slate-500 dark:text-slate-400 group-hover:text-industrial-orange transition-colors">
                                            {domainIcons[domain.subject] || <Gauge className="w-5 h-5" />}
                                        </div>
                                        <div className="text-right">
                                            <div className="text-[7px] font-mono text-slate-500 uppercase tracking-widest mb-0.5">UNIT_PROFICIENCY</div>
                                            <div className="text-base font-heading font-black text-industrial-concrete dark:text-white group-hover:text-industrial-orange transition-colors">{domain.A}%</div>
                                        </div>
                                    </div>
                                    <div className="text-[10px] font-industrial text-slate-600 dark:text-slate-300 uppercase tracking-widest font-bold transition-colors">{domain.subject}</div>
                ...
                {/* BOTTOM: Full Width Telemetry */}
                <div className="w-full">
                    <motion.div
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        className="glass rounded-sm border-[4px] border-slate-300 dark:border-slate-800 shadow-2xl relative overflow-hidden group/hmi transition-all duration-500 hover:border-industrial-orange/30"
                    >
                        <ScanlineEffect />
                        ...
                        <div className="p-8 relative z-30">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                                <div>
                                    <h3 className="text-3xl font-heading font-black text-industrial-orange mb-2 flex items-center gap-3">
                                        <Activity className="w-8 h-8 animate-pulse" />
                                        HMI TELEMETRY_STREAMING
                                    </h3>
                                    <p className="text-slate-500 text-[10px] font-mono uppercase tracking-[0.3em]">
                                        Central Hub Node: Operational Intelligence System // Buffer: 4.2ms // {new Date().toLocaleDateString()}
                                    </p>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="px-4 py-2 bg-green-500/10 border border-green-500/30 text-green-500 text-xs font-mono font-bold uppercase flex items-center gap-3">
                                        <span className="w-2 h-2 rounded-full bg-green-500 animate-ping"></span>
                                        CORE_UPLINK_STABLE
                                    </div>
                                    <div className="hidden lg:flex flex-col items-end">
                                        <span className="text-[8px] font-mono text-slate-600 uppercase">Packet_Loss: 0.001%</span>
                                        <span className="text-[8px] font-mono text-slate-600 uppercase">Jitter: 0.12ms</span>
                                    </div>
                                </div>
                            </div>

                            {/* Main Chart Area - Significantly Wider and Taller */}
                            <div className="h-[400px] w-full bg-slate-950/50 border border-slate-800 rounded-sm p-6 relative mb-10 overflow-hidden group shadow-inner">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={liveData}>
                                        ...
                                        <RechartsTooltip
                                            contentStyle={{ backgroundColor: 'rgba(0,0,0,0.95)', border: '1px solid #FF6B35', borderRadius: '0px', padding: '16px' }}
                                            labelStyle={{ display: 'none' }}
                                            itemStyle={{ fontSize: '11px', color: '#fff', textTransform: 'uppercase', fontFamily: 'monospace' }}
                                        />
                                        <Line
                                            type="stepAfter"
                                            dataKey="value"
                            ...
                            {/* Console Controls - Horizontal Layout for Wide View */}
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                <div className="p-6 glass border border-slate-300 dark:border-slate-800 rounded-sm transition-colors lg:col-span-2">
                                    <div className="text-[11px] font-industrial font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center justify-between">
                                        <span className="flex items-center gap-2"><Settings className="w-4 h-4 animate-spin-slow" /> Simulation_Controller</span>
                                        <span className="font-mono text-[9px]">Module: OIP-728</span>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div>
                                            <div className="text-[9px] text-slate-600 dark:text-slate-500 font-mono mb-3 uppercase tracking-wider">Sampling_Frequency</div>
                                            <div className="flex gap-2">
                                                {[5000, 3000, 1000].map(speed => (
                                                    <button
                                                        key={speed}
                                                        onClick={() => setSimSpeed(speed)}
                                                        className={`flex-1 py-2 text-[10px] font-mono font-bold border transition-all ${simSpeed === speed
                                                            ? 'bg-industrial-orange border-industrial-orange text-slate-950 shadow-[0_0_10px_rgba(255,107,53,0.4)]'
                                        ...
                                        <div>
                                            <div className="text-[9px] text-slate-500 font-mono mb-3 uppercase tracking-wider">Amplitude_Gain_Factor</div>
                                            <div className="flex gap-2">
                                                {[0.5, 1, 2].map(level => (
                                                    <button
                                                        key={level}
                                                        onClick={() => setIntensity(level)}
                                                        className={`flex-1 py-2 text-[10px] font-mono font-bold border transition-all ${intensity === level
                                                            ? 'bg-industrial-blue border-industrial-blue text-slate-950 shadow-[0_0_10px_rgba(56,189,248,0.4)]'
                                ...
                                <div className="flex flex-col gap-4 lg:col-span-1">
                                    <div className="p-6 bg-industrial-orange/5 border border-industrial-orange/20 rounded-sm flex-1 flex flex-col justify-center">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-[10px] font-industrial font-bold text-industrial-orange uppercase tracking-[0.2em]">Operational_Stability</span>
                                            <span className="text-2xl font-mono font-black text-industrial-orange dark:text-white transition-colors">{(96.8 + Math.random() * 2).toFixed(1)}%</span>
                                        </div>
                                        <div className="h-3 w-full bg-slate-900 rounded-none p-0.5 border border-slate-800 overflow-hidden">
                                            <motion.div
                                    ...
                                    <div className="p-4 bg-slate-100 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-800 flex items-center justify-between transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-2.5 h-2.5 rounded-full ${diagnosticStatus === 'Ready' ? 'bg-industrial-orange animate-pulse' : 'bg-industrial-blue animate-ping'}`}></div>
                                            <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400 uppercase tracking-widest transition-colors">SYSTEM_DIAGNOSTIC</span>
                                        </div>
                                        <span className={`text-xs font-mono font-bold uppercase ${diagnosticStatus === 'Ready' ? 'text-green-500' : 'text-industrial-blue'}`}>
                                            {diagnosticStatus}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div className="absolute top-3 right-4 opacity-10 font-mono text-[9px] text-white uppercase tracking-widest">Global-DS-HMI-042/X-RAY</div>
                    </motion.div>
                </div>
            </div>
        </section>
